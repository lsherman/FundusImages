function Rec = Vasc_detection_rework_DRIVE_classify_DRIVE_trn(flname)
...
Yg=regionprops(g0,'PixelList');
pixg=cat(1,Yg.PixelList);
...
for j = 1:1
    tst_ind=1;
    fprintf('Testing image number= %d\r',j);
    evalfile = deblank(testfiles{j});
    I = imread(flname);
    %I = imread('Data\test\images\tst.png');
    K=(I(:,:,2));
    ...
    Rec=bwareaopen(Rec,30).*g0;
    %% Timing analysis
    timing(j)=toc(tstart);
    %% Save the finally segmented image
    path='Data\DRIVE segmented\';
    save2=[path evalfile '.tif'];
    imwrite(Rec,save2);
